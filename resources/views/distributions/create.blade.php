...
<head>
    ...
    <title>Nouvelle Distribution - DonaLink</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alert-warning-custom {
            background: #f8fbff; border: 1.5px solid #dce8f5;
            border-radius: 12px; padding: 16px 18px;
            color: #08346b; font-size: 0.88rem; font-weight: 500;
            display: flex; align-items: center; gap: 10px;
        }
        .alert-warning-custom a { color: #06142f; font-weight: 700; }
...
<div class="main-content">
    ...
    <div class="page-header">
        <div>
            <h4>Nouvelle Distribution</h4>
            <small>Redistribuer un don à un bénéficiaire</small>
        </div>
    ...
    <div class="form-card">
        ...
        @if($dons->isEmpty())
            <div class="alert-warning-custom">
                ⚠️ Aucun don confirmé disponible pour une distribution.
                <a href="/dons">Confirmer des dons d'abord.</a>
            </div>
        @else
        ...
        <form action="/distributions" method="POST">
            ...
            <div class="section-title">Don à redistribuer</div>

            <div class="mb-3">
                <label class="form-label">Sélectionner un don</label>
                <select name="don_id" class="form-select @error('don_id') is-invalid @enderror">
                    <option value=""> Choisir un don </option>
                    @foreach($dons as $don)
                        <option value="{{ $don->id }}" {{ old('don_id') == $don->id ? 'selected' : '' }}>
                            @if($don->type == 'financier')
                                 {{ number_format($don->montant, 0, ',', ' ') }} F
                            @else
                                 {{ $don->description_materiel }} (x{{ $don->quantite }})
                            @endif
                            — {{ $don->campagne->titre }}
                        </option>
                    @endforeach
                </select>
                @error('don_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="section-title">Bénéficiaire</div>

            <div class="mb-3">
                <label class="form-label">Sélectionner un bénéficiaire</label>
                <select name="beneficiaire_id" class="form-select @error('beneficiaire_id') is-invalid @enderror">
                    <option value=""> Choisir un bénéficiaire </option>
                    @foreach($beneficiaires as $beneficiaire)
                        <option value="{{ $beneficiaire->id }}" {{ old('beneficiaire_id') == $beneficiaire->id ? 'selected' : '' }}>
                ...
                @error('beneficiaire_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    ...
                    @error('date_distribution') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
            ...
            <div class="form-actions">
                <button type="submit" class="btn-submit">
                    <i class="fas fa-save"></i> Enregistrer la distribution
                </button>
                <a href="/distributions" class="btn-cancel">Annuler</a>
            </div>
