...
<head>
    ...
    <title>Modifier Distribution - DonaLink</title>
...
<div class="main-content">
    ...
    <div class="page-header">
        <div>
            <h4>Modifier la Distribution</h4>
            <small>Mise à jour des informations</small>
        </div>
    ...
    <div class="form-card">
        <form action="/distributions/{{ $distribution->id }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select name="don_id" class="form-select @error('don_id') is-invalid @enderror">
                    @foreach($dons as $don)
                        <option value="{{ $don->id }}"
                            {{ old('don_id', $distribution->don_id) == $don->id ? 'selected' : '' }}>
                ...
                <select name="beneficiaire_id" class="form-select @error('beneficiaire_id') is-invalid @enderror">
                    @foreach($beneficiaires as $beneficiaire)
                        <option value="{{ $beneficiaire->id }}"
                            {{ old('beneficiaire_id', $distribution->beneficiaire_id) == $beneficiaire->id ? 'selected' : '' }}>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    ...
                    <input type="date" name="date_distribution"
                        ...
                        value="{{ old('date_distribution', $distribution->date_distribution) }}">
                    @error('date_distribution') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">Notes</label>
                    <textarea name="notes" rows="1"
                        class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $distribution->notes) }}</textarea>
